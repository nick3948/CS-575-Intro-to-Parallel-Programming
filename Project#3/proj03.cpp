...
#pragma omp parallel for default(none) shared(Cities, Capitals, stderr)
		for (int i = 0; i < NUMCITIES; i++)
		{
			int capitalnumber = -1;
...
#pragma omp critical
	...
	for (int k = 0; k < NUMCAPITALS; k++)
	{
		...
		{
			float dx = Capitals[k].longitude - Cities[m].longitude;
			float dy = Capitals[k].latitude - Cities[m].latitude;
		...
		Capitals[k].name = Cities[Index].name;
	}
